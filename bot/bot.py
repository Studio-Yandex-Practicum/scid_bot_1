...
ADMIN_IDS = [123456789, 225735315]  # Замените на реальные Telegram ID администраторов
...
async def send_welcome(message: types.Message):
    if is_admin(message.from_user.id):
        await message.answer(buttons_structure['admin_block_start_message'], reply_markup=generate_main_menu())
    else:
...
async def process_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not is_admin(user_id):
        await callback.answer("У вас нет доступа к этим функциям.", show_alert=True)
        return
    ...
    if data == 'create_button':
        ...
        await callback.message.answer("Введите `callback_data` кнопки, контент которой хотите получить:")
        await state.set_state(CreateButtonStates.waiting_for_callback)
    elif data == 'get_button_subs':
        if buttons_storage:
            buttons_list = "\n".join([f"Callback Data: {key}, Text: {value}" for key, value in buttons_storage.items()])
            await callback.message.answer(f"Все дочерние кнопки:\n{buttons_list}")
        else:
...
async def process_button_text(message: types.Message, state: FSMContext):
    ...
    if not button_text:
        await message.answer("Текст кнопки не может быть пустым. Введите текст для новой кнопки:")
        return
...
async def process_button_callback(message: types.Message, state: FSMContext):
    ...
    if not callback_data:
        await message.answer("`callback_data` не может быть пустым. Введите `callback_data` для этой кнопки:")
        return

    if callback_data in buttons_storage:
        await message.answer("Такая `callback_data` уже существует. Попробуйте снова.")
        await state.clear()
        return
